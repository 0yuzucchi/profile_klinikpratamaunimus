    ...
    if (hasDropdown) {
        return (
            <div className={`${desktopBase} ${isActive ? desktopActive : desktopInactive}`}>
                ...
                {isActive && (
                    <span className="absolute bottom-0 left-0 w-full h-1 bg-[#00994d] rounded-t-full"></span>
                )}
...
export default function Navbar() {
    ...
    const navItems = useMemo(() => [
        ...
        // --- PERUBAHAN DI SINI: MENJADI DROPDOWN ---
        { 
            label: 'FASILITAS', 
            icon: MenuIcons.Facility,
            href: '#', // Placeholder untuk parent
            children: [
                { href: route('facilities.care'), label: 'Fasilitas Perawatan' },
                { href: route('facilities.support'), label: 'Fasilitas Penunjang' }
            ]
        },
        // ------------------------------------------
    ...
    // Fungsi canggih untuk mengecek aktif (support dropdown)
    const isItemActive = (item, currentUrl) => {
        const currentParts = parseHrefToParts(currentUrl);

        // Jika punya children, cek apakah salah satu anaknya aktif
        if (item.children) {
        ...
        // Home Page dengan hash
        if (itemParts.pathname === '/' && currentParts.pathname === '/' && 
            itemParts.hash && currentParts.hash && 
            itemParts.hash === currentParts.hash) {
            return true;
        }
            ...
            <div className="bg-white border-b md:border-none shadow-sm md:shadow-none relative z-50">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-2 md:pb-4 flex justify-between items-center relative">
                    ...
                    <div className="flex items-center gap-3">
                        <Link href={route('home')} onClick={() => handleLinkClick(route('home'))}>
                            <img 
                    ...
                    <div className="hidden md:flex items-center gap-8 h-full">
                        ...
                        <Link href={route('registration.index')} onClick={() => handleLinkClick(route('registration.index'))} className="bg-[#00b050] hover:bg-[#00994d] text-white font-bold py-5 px-8 rounded-b-2xl rounded-t-none shadow-md transition-colors uppercase tracking-wide self-start -mt-8">
                            DAFTAR ONLINE
                        </Link>
                    </div>
                        ...
                        >
                                        ...
                                        return (
                                            <li key={item.label}>
                                                <NavLink
                                                    ...
                                                    isActive={active}
                                                    hasDropdown={true}
                                                    onClick={(e) => { e.preventDefault(); toggleMobileSubmenu(item.label); }}
                                                    icon={item.icon}>
                                                    {item.label}
                                                    {/* Rotate icon on open */}
                                                        ...
                                                        >
                                                            {item.children.map(child => (
                                                                <Link
                                                                    ...
                                                                    href={child.href}
                                                                    onClick={() => handleLinkClick(child.href)}
                                                                    className="block pl-16 pr-6 py-3 text-gray-600 hover:text-[#00994d] text-sm font-medium border-l-4 border-transparent hover:border-[#00994d]"
                                                                >
                                                                    {child.label}
                                                                </Link>
                                                            ))}
                            ...
                            <div className="p-6 mt-auto bg-gray-50 border-t space-y-4">
                                <Link
                                    href="/pendaftaran"
                                    onClick={() => setMobileMenuOpen(false)}
                                    className="block w-full text-center bg-[#00b050] text-white font-bold py-3 rounded"
                                >
                                    DAFTAR ONLINE
                                </Link>
